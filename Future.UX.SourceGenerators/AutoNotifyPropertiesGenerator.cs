...
namespace Future.UX.SourceGenerators
{
    ...
    public sealed class AutoNotifyPropertiesGenerator : IIncrementalGenerator
    {
        ...
        {{
            ...
            {{
                ...
                OnPropertyChanged(nameof({propName}));");
                        ...
                        // Appending dependent computed properties
...
");
                    ...
                    if (generatedProperties.Count > 0)
                    {
                        ...
                        foreach (var gp in generatedProperties.OrderBy(x => x))
                        {
                            ...
                            sb.Append($@"
        partial void On{gp}Changing({typeName}? oldValue, {typeName}? newValue, ref bool cancel);
        partial void On{gp}Changed({typeName}? oldValue, {typeName}? newValue);
");
                        }
                    }
...
{nsClose}";
                    ...
                    spc.AddSource($"{className}.AutoProperties.g.cs", SourceText.From(src, Encoding.UTF8));
                }
            });
        }
